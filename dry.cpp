...
void Dry::createDialog(QSqlTableModel *model, int row)
{
    ...
    year_lbl->setText("Рік:" + model->index(row, 6).data().toString());
    price_lbl->setText("Ціна:" + model->index(row, 7).data().toString());
    dtp_lbl->setText("Участь в ДТП:" + model->index(row, 8).data().toString());
    stan_lbl->setText("Стан:" + model->index(row, 10).data().toString());
    kpp_lbl->setText("Тип коробки передач:" + model->index(row, 11).data().toString());
    fuel_lbl->setText("Тип палива:" + model->index(row, 12).data().toString());
    drive_lbl->setText("Тип приводу:" + model->index(row, 13).data().toString());
    fuelcost_lbl->setText("Витрата палива на 100 км:" + model->index(row, 14).data().toString());
    capacity_lbl->setText("Об'єм двигуна:" + model->index(row, 15).data().toString());
    power_lbl->setText("Потужність двигуна:" + model->index(row, 16).data().toString());
    run_lbl->setText("Пробіг:" + model->index(row, 17).data().toString());
    doors_lbl->setText("Кількість дверей:" + model->index(row, 18).data().toString());
    places_lbl->setText("Кількість місць:" + model->index(row, 19).data().toString());
    color_lbl->setText("Колір:" + model->index(row, 20).data().toString());
    vincode_lbl->setText("Вінкод:" + model->index(row, 21).data().toString());

    QPixmap pix(model->index(row, 9).data().toString());
    picture_lbl->setPixmap(pix.scaled(400, 400, Qt::KeepAspectRatio));
